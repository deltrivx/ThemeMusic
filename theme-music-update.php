...
function ucwc_http_get_once($url, $timeout = 12) {
    ...
    $opts = [
        ...
        CURLOPT_MAXREDIRS => 5,
        CURLOPT_CONNECTTIMEOUT => max(3, min(8, (int)$timeout)),
...
function ucwc_http_get_quick($url, $budget = 7) {
    // Changelog/status requests must not wait through every mirror and retry.
    $deadline = microtime(true) + max(3, (int)$budget);
    $errors = [];
    $candidates = array_slice(ucwc_url_candidates($url), 0, 3);
    foreach ($candidates as $try) {
        $left = $deadline - microtime(true);
        if ($left <= 0) break;
        [$data, $err, $code] = ucwc_http_get_once($try, max(3, (int)ceil($left)));
        if ($data !== false) return [$data, "", $code];
        $errors[] = $try . " → " . $err;
    }
    return [false, implode(" | ", $errors) ?: "远程请求超时", 0];
}
...
function ucwc_fetch_index($repo_raw) {
    ...
    [$data, $err] = ucwc_http_get_quick(rtrim($repo_raw, "/") . "/versions/index.json?_ts=" . time(), 7);
    if ($data === false) {
...
function ucwc_fetch_changelog($repo_raw) {
    $cache = "/tmp/theme-music-changelog-cache.md";
    $ttl = 300;
    if (is_file($cache) && (time() - (int)@filemtime($cache)) < $ttl) {
        $cached = @file_get_contents($cache);
        if (is_string($cached) && trim($cached) !== "") return [$cached, ""];
    }
    [$data, $err] = ucwc_http_get_quick(rtrim($repo_raw, "/") . "/CHANGELOG.md?_ts=" . time(), 7);
    if ($data === false) {
        if (is_file($cache)) {
            $cached = @file_get_contents($cache);
            if (is_string($cached) && trim($cached) !== "") return [$cached, "GitHub 响应较慢，已使用缓存更新日志。"];
        }
        return ["", "拉取更新日志失败（GitHub 请求超时或暂时不可达）：$err"];
    }
    @file_put_contents($cache, $data);
    return [$data, ""];
}
...
function ucwc_run_install($repo_raw, $upd_log, $mode, $version = "", $install_mode = "ota") {
    ...
    $msg = $ok
        ...
        : ("操作失败（exit $code）。" . ($text !== "" ? " " . mb_substr($text, -1200) : " 未返回安装日志。"));
...
if (!empty($_POST["UCWC_ACTION"])) {
    $action = strtolower(trim((string)$_POST["UCWC_ACTION"]));
} elseif (!empty($_GET["UCWC_ACTION"])) {
    $action = strtolower(trim((string)$_GET["UCWC_ACTION"]));
}

if ($action === "") {
    ucwc_json_out(["ok" => false, "error" => "缺少 UCWC_ACTION", "action" => "", "status" => 400], 400);
}
...
if ($action === "install_latest" || $action === "install_version" || $action === "uninstall") {
    ...
    if ($action !== "install_latest" && $action !== "install_version" && $action !== "uninstall") {
        ucwc_json_out(["ok" => false, "error" => "未知安装动作。", "action" => $action, "status" => 400], 400);
    }
        ...
        if ($pid !== "" && ctype_digit($pid)) {
            ...
            if (!$alive && $age > 2) {
                ...
                if (is_file($paths["log"])) {
                    $rawTail = (string)@file_get_contents($paths["log"]);
                    $lines = preg_split('/\r?\n/', trim($rawTail));
                    $tail = $lines ? trim((string)end($lines)) : "";
                }
                $meta["message"] = "安装进程已退出（异常结束）。" . ($tail !== "" ? " " . mb_substr($tail, -400) : "");
                ucwc_job_write_meta($job, $meta);
            }
        }
    }
