...
(function (global) {
  ...
  function buildUi() {
    ...
    /*
     * Transport + side tools: event delegation on the card root.
     * Unraid/DOM rebind can replace button nodes; delegated handlers stay alive.
     * Guard with data flag so rebuildUi re-entry never stacks listeners.
     */
    if (!root.getAttribute("data-ucwc-bound")) {
      root.setAttribute("data-ucwc-bound", "1");
        ...
        function (ev) {
          try {
            ...
            var btn =
              t.closest && t.closest
                ? t.closest(
                    ".ucwc-music-btn, .ucwc-music-side-btn, .ucwc-music-info-btn, .ucwc-music-close"
                  )
                : null;
