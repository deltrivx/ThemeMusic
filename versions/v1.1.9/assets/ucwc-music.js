/**
 * ThemeMusic player — dashboard card + sitewide engine
 ...
 * Cross-page: the visible chip stays embedded in Tower while one fixed-name,
 * same-origin audio host owns sitewide playback across full page reloads.
 * The host can only be opened by an explicit playback gesture and is never
 * recreated by a timer, heartbeat or navigation callback.
 */
(function (global) {
  "use strict";
  ...
  var HOST_CHANNEL = "ucwc-theme-music-host-v2";
  var HOST_URL = "/plugins/theme.music/assets/ucwc-music-host.html?v=2";
  var HOST_HEART_KEY = "ucwc_music_host_v2_heartbeat";
  var HOST_OPEN_LOCK_KEY = "ucwc_music_host_v2_open_lock";
  var HOST_CLIENT_KEY = "ucwc_music_host_v2_client";
  var hostBc = null;
  var hostWindow = null;
  var hostState = null;
  var hostLastSeen = 0;
  var hostClientId = "";
  var hostBound = false;
  var hostHeartbeatTimer = 0;
  var hostPendingCommand = null;
  var hostOpening = false;
  function rawCfg() {
  ...
  function enabled() {
    ...
    return on && (source === "local" || source === "navidrome");
  }
  ...
  function persistentHostClientId() {
    if (hostClientId) return hostClientId;
    try { hostClientId = sessionStorage.getItem(HOST_CLIENT_KEY) || ""; } catch (e0) {}
    if (!hostClientId) {
      hostClientId = "tower-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 9);
      try { sessionStorage.setItem(HOST_CLIENT_KEY, hostClientId); } catch (e1) {}
    }
    return hostClientId;
  }

  function towerPageVisible() {
    try { return document.visibilityState !== "hidden"; } catch (e0) { return true; }
  }

  function persistentHostHeartbeatFresh() {
    try {
      var raw = localStorage.getItem(HOST_HEART_KEY);
      if (!raw) return false;
      var beat = JSON.parse(raw);
      return !!(beat && Number(beat.ts) > 0 && Date.now() - Number(beat.ts) < 3200);
    } catch (e0) {
      return false;
    }
  }

  function persistentHostActive() {
    return !!(isSitewidePlay() && hostState && Date.now() - hostLastSeen < 4000);
  }

  function persistentHostExpected() {
    return !!(isSitewidePlay() && (persistentHostActive() || hostOpening || persistentHostHeartbeatFresh()));
  }

  function hostBaseMessage(type) {
    return {
      protocol: HOST_PROTOCOL,
      channel: HOST_CHANNEL,
      from: "tower",
      type: type || "heartbeat",
      clientId: persistentHostClientId(),
      visible: towerPageVisible(),
      index: state.index,
      id: current() && current().id ? current().id : "",
      t: pendingSeekTo > 0.5 ? pendingSeekTo : 0,
      volume: state.volume,
  ...
  function postToPersistentHost(message) {
    if (!message) return false;
    message.protocol = HOST_PROTOCOL;
    message.channel = HOST_CHANNEL;
    message.from = "tower";
    message.clientId = persistentHostClientId();
    if (typeof message.visible !== "boolean") message.visible = towerPageVisible();
    message.ts = Date.now();
    try {
      if (hostBc) {
        hostBc.postMessage(message);
        return true;
      }
    } catch (e0) {}
    try {
      if (hostWindow && !hostWindow.closed) {
        hostWindow.postMessage(message, location.origin);
        return true;
      }
    } catch (e1) {}
    return false;
  ...
  function hostConfigMessage(type) {
    var message = hostBaseMessage(type || "config");
    message.tracks = state.tracks.slice(0, 1200);
    if (hostState && typeof hostState.t === "number") message.t = hostState.t;
    return message;
  }

  function syncPersistentHostConfig() {
    if (!isSitewidePlay() || !state.tracks.length) return;
    postToPersistentHost(hostConfigMessage("config"));
  }

  function applyPersistentHostState(data) {
    if (!data || data.protocol !== HOST_PROTOCOL || data.from !== "host") return;
    if (data.type === "host-gone") {
      hostState = null;
      hostLastSeen = 0;
      hostOpening = false;
      try { localStorage.removeItem(HOST_HEART_KEY); } catch (eGone) {}
      try { updatePlayBtn(); updateChipUi(); } catch (eGoneUi) {}
      return;
    }
    var previousIndex = state.index;
    var previousShuffle = state.shuffle;
    var previousRepeat = state.repeat;
    hostLastSeen = Date.now();
    hostState = data;
    hostOpening = false;
    if (typeof data.index === "number" && data.index >= 0 && data.index < state.tracks.length) {
      state.index = data.index;
    } else if (data.id && state.tracks.length) {
      for (var i = 0; i < state.tracks.length; i++) {
        if (state.tracks[i] && state.tracks[i].id === data.id) { state.index = i; break; }
      }
    }
    ...
    if (data.repeat === "off" || data.repeat === "one" || data.repeat === "all") state.repeat = data.repeat;
    state.playing = !!data.playing;
    resumeIntent = !!data.intent;
    if (typeof data.t === "number" && data.t >= 0) pendingSeekTo = data.t;
    if (audio && !audio.paused) {
      playAttemptToken++;
      try { audio.pause(); } catch (e0) {}
    }
    if (hostPendingCommand) {
      var pending = hostPendingCommand;
      hostPendingCommand = null;
      postToPersistentHost(pending);
    }
    try {
      ...
      if (previousShuffle !== state.shuffle || previousRepeat !== state.repeat) updateModeBtns();
      updatePlayBtn();
      syncSitewideChip();
      updateChipUi();
      onTime();
  ...
  function bindPersistentHost() {
    if (hostBound || !isSitewidePlay()) return;
    hostBound = true;
    persistentHostClientId();
    try {
      hostBc = new BroadcastChannel(HOST_CHANNEL);
      hostBc.onmessage = function (ev) { applyPersistentHostState(ev && ev.data); };
    } catch (e0) { hostBc = null; }
    window.addEventListener("message", function (ev) {
      if (ev.origin !== location.origin || !ev.data || ev.data.channel !== HOST_CHANNEL) return;
      applyPersistentHostState(ev.data);
    });
    postToPersistentHost(hostBaseMessage("hello"));
    hostHeartbeatTimer = setInterval(function () {
      if (!isSitewidePlay()) return;
      postToPersistentHost(hostBaseMessage("heartbeat"));
    }, 1000);
  }

  /**
   * The only host creation path. It requires a real playback gesture, uses a
   * fixed window name, and is protected by a short cross-tab lock. No timer or
   * lifecycle callback calls window.open().
   */
  function ensurePersistentHostFromGesture(command) {
    if (!isSitewidePlay()) return false;
    bindPersistentHost();
    if (persistentHostActive() || persistentHostHeartbeatFresh()) {
      postToPersistentHost(command);
      return true;
    }
    var now = Date.now();
    var lock = null;
    try { lock = JSON.parse(localStorage.getItem(HOST_OPEN_LOCK_KEY) || "null"); } catch (e0) {}
    if (lock && Number(lock.ts) > 0 && now - Number(lock.ts) < 2200 && lock.clientId !== persistentHostClientId()) {
      postToPersistentHost(hostBaseMessage("hello"));
      setStatus("全局音乐宿主正在启动，请稍候再点一次");
      return true;
    }
    try {
      localStorage.setItem(HOST_OPEN_LOCK_KEY, JSON.stringify({ clientId: persistentHostClientId(), ts: now }));
    } catch (e1) {}
    ...
    try {
      hostWindow = window.open(
        HOST_URL,
        HOST_NAME,
        "popup=yes,width=380,height=112,resizable=yes,scrollbars=no"
      );
    } catch (e2) { hostWindow = null; }
    if (!hostWindow) {
      hostOpening = false;
      hostPendingCommand = null;
      setStatus("浏览器拦截了音乐宿主窗口，请允许此站点弹窗后重试");
      return true;
    }
    [120, 360, 800, 1500].forEach(function (delay) {
      setTimeout(function () {
  ...
  function dispatchPersistentHost(command, fromGesture) {
    if (!isSitewidePlay()) return false;
    bindPersistentHost();
    command = Object.assign(hostConfigMessage(command.type || command.cmd || "config"), command);
    if (persistentHostActive() || persistentHostHeartbeatFresh()) {
      postToPersistentHost(command);
      return true;
    }
    if (!fromGesture) return false;
    return ensurePersistentHostFromGesture(command);
  }
  ...
  function isUiPlaying() {
    if (persistentHostActive()) return !!hostState.playing;
  ...
  function savePlaySession(forcePlaying) {
    try {
      var t = current();
      var hostLive = persistentHostActive();
      var livePlaying = hostLive ? !!hostState.playing : !!(audio && !audio.paused);
      if (livePlaying) resumeIntent = true;
      var playing = typeof forcePlaying === "boolean" ? forcePlaying : livePlaying;
      var id = hostLive && hostState.id ? hostState.id : t && t.id ? t.id : "";
      var curT = 0;
      if (hostLive && typeof hostState.t === "number" && hostState.t >= 0) curT = hostState.t;
      if (pendingSeekTo > 0.5 && id) curT = pendingSeekTo;
      if (!hostLive && audio && isFinite(audio.currentTime) && audio.currentTime > 0.25) {
      ...
      var idx = hostLive && typeof hostState.index === "number" ? hostState.index : state.index;
  ...
  function savePlaySessionForNav() {
    try {
      ...
      lastNavSave = now;
      var live = persistentHostActive() ? !!hostState.playing : !!(audio && !audio.paused);
    ...
    try {
      if (isSitewidePlay()) {
        ...
        if (reason === "visibility") {
          postToPersistentHost(Object.assign(hostBaseMessage("visibility"), { visible: false }));
        } else {
          // A full Tower navigation briefly hides the old document. Navigation
          // grace keeps the fixed host alive until the replacement page says hello.
          postToPersistentHost(Object.assign(hostBaseMessage("navigation"), { visible: false }));
        }
        savePlaySessionForNav();
        return;
      }
      // Card-only playback belongs to the dashboard document and stops when
      // that document is hidden or replaced.
  ...
  function bindMediaSession() {
    try {
      ...
      navigator.mediaSession.setActionHandler("pause", function () {
        if (persistentHostActive()) dispatchPersistentHost({ type: "pause", userAction: true }, false);
  ...
  function libraryStatusText() {
    ...
    if (!n) return state.loaded ? "目录内无支持的音频" : "";
    var base = "共 " + n + " 首";
  ...
  function updatePlayBtn() {
    ...
    if ((audio || persistentHostActive()) && shouldShowCard()) {
      try {
        var cur = persistentHostActive() ? Number(hostState.t) || 0 : audio.currentTime || 0;
        var dur = persistentHostActive() ? Number(hostState.duration) || 0 : audio.duration || 0;
        paintProgressTo(document.getElementById("ucwc-music-card") || root, cur, dur);
      } catch (e1) {}
    }
    updateChipUi();
  ...
  function lyricClockMs() {
    if (!audio && !persistentHostActive()) return 0;
    var cur = persistentHostActive() && typeof hostState.t === "number" ? hostState.t : audio.currentTime;
  ...
  function syncLyrics(force) {
    if (!audio && !persistentHostActive()) return;
  ...
  function onTime() {
    ...
    var cur = hostLive && typeof hostState.t === "number" ? hostState.t : audio.currentTime || 0;
    var dur = hostLive && typeof hostState.duration === "number" ? hostState.duration : audio.duration || 0;
    var liveCard = null;
    try {
  ...
  function playAt(idx, autoPlay, startAt) {
    ...
    if (isSitewidePlay()) {
      if (persistentHostActive()) {
        dispatchPersistentHost(
          { type: "playAt", index: idx, autoPlay: autoPlay !== false, t: typeof startAt === "number" ? startAt : 0 },
          false
        );
      }
      return;
  ...
  function tryResumeFromSession(forcePlay) {
    ...
    if (isSitewidePlay()) {
      bindPersistentHost();
      syncPersistentHostConfig();
      return persistentHostExpected();
    }
  ...
  function togglePlay() {
    ...
    if (isSitewidePlay()) {
      if (persistentHostActive()) dispatchPersistentHost({ type: "toggle" }, false);
      return;
    }
    var a = ensureAudio();
    if (!a.src) {
  ...
  function runTransportAction(action, ev, userGesture) {
    ...
    if (isSitewidePlay()) {
      var command = null;
      if (action === "play") {
        command = persistentHostActive()
          ? { type: "toggle" }
          : { type: "playAt", index: state.index, autoPlay: true, t: pendingSeekTo > 0.5 ? pendingSeekTo : 0 };
      } else if (action === "prev" || action === "next") {
        command = { type: action };
      }
      if (command && dispatchPersistentHost(command, fromGesture)) return true;
    }
    if (action === "prev") {
      prev();
      return true;
  ...
  function next(fromEnded) {
    if (!state.tracks.length) return;
    if (isSitewidePlay()) {
      if (persistentHostActive()) dispatchPersistentHost({ type: "next", fromEnded: !!fromEnded }, false);
      return;
    }
    pendingSeekTo = 0;
    resumeLockId = "";
  ...
  function prev() {
    if (!state.tracks.length) return;
    if (isSitewidePlay()) {
      if (persistentHostActive()) dispatchPersistentHost({ type: "prev" }, false);
      return;
    }
  ...
  function runCardAction(btn, ev) {
    ...
    if (btn.classList.contains("ucwc-music-item")) {
      ...
      if (isFinite(itemIndex) && itemIndex >= 0 && itemIndex < state.tracks.length) {
        if (dispatchPersistentHost({ type: "playAt", index: itemIndex, autoPlay: true, t: 0 }, true)) return true;
  ...
  function commitCardSeek(input) {
    ...
    seeking = false;
    seekingSince = 0;
    var hostDuration = persistentHostActive() && typeof hostState.duration === "number" ? hostState.duration : 0;
    var duration = hostDuration > 0 ? hostDuration : audio && isFinite(audio.duration) ? audio.duration : 0;
    ...
    if (persistentHostActive()) dispatchPersistentHost({ type: "seek", t: target }, false);
  ...
  function restoreClientLibraryCache() {
    try {
      ...
      state.tracks = cached.tracks.slice(0, 1200);
  ...
  function fetchList(forceScan) {
    ...
    setStatus(sourceLabelText() === "Navidrome" ? "正在加载 Navidrome 曲库…" : "正在扫描本地曲库…");
  ...
  function maybeResumeOrAutoplay() {
    if (!state.tracks.length) return;
    if (isSitewidePlay()) {
      // The embedded Tower UI is only a remote in sitewide mode. Never create
      // a replacement local Audio element during refresh or soft navigation.
      bindPersistentHost();
      syncPersistentHostConfig();
      syncSitewideChip(persistentHostExpected() ? "" : "点击播放启动全局播放");
      showResumeChip(persistentHostExpected() ? "" : "点击播放启动全局播放");
      return;
    }
  ...
  function mount() {
    ...
    applyCfgDefaults();
    if (isSitewidePlay()) bindPersistentHost();
    ...
    if (!state.loaded) {
      ...
      if (!restoreClientLibraryCache()) fetchList();
    ...
    try {
      ...
      document.addEventListener("visibilitychange", function () {
        if (document.visibilityState === "hidden") onDocumentHidden("visibility");
          ...
          postToPersistentHost(Object.assign(hostBaseMessage("visibility"), { visible: true }));
        }
      });
      window.addEventListener(
        "resize",
