...
(function () {
  ...
  function alignCtrlWidth() {
    ...
    if (!(w > 40)) return;
    w = Math.round(w);
    /* Desktop controls align to the volume row; mobile rules override this with 100%. */
    if (w < 420) w = 420;
    if (w > 420) w = 420;
    var px = w + "px";
    ...
    // Keep the measured column on desktop; mobile controls must remain fluid.
    var isMobile = false;
    try { isMobile = window.matchMedia && window.matchMedia("(max-width: 720px)").matches; } catch (eMedia) {}
    try {
      var allSel = form.querySelectorAll("dd > select, select");
      for (i = 0; i < allSel.length; i++) {
        if (!allSel[i].style) continue;
        if (isMobile) {
          allSel[i].style.setProperty("width", "100%", "important");
          allSel[i].style.setProperty("min-width", "0", "important");
        } else {
    ...
    var mobileInputs = form.querySelectorAll("#tm-navidrome-url, #tm-navidrome-user, #tm-navidrome-password, #tm-fnos-url, #tm-fnos-user, #tm-fnos-password");
    for (i = 0; i < mobileInputs.length; i++) {
      if (isMobile) {
        mobileInputs[i].style.setProperty("width", "100%", "important");
        mobileInputs[i].style.setProperty("min-width", "0", "important");
        mobileInputs[i].style.setProperty("max-width", "100%", "important");
      }
    }
    ...
    if (vol) {
      if (isMobile) {
        ...
        vol.style.setProperty("width", "420px", "important");
      }
      vol.style.setProperty("min-width", "0", "important");
    }
  }
  ...
  function showChangelog(data, preselect) {
    ...
    var sel =
      ...
      "";
    cache.selected = sel;
    renderChangelog(data.changelog_error || "");
  }

  function renderChangelog(changelogError) {
    ...
    var warning = changelogError
      ? '<p class="tm-err">' + esc(changelogError) + "</p>"
      : "";
    body.innerHTML =
      warning +
  ...
  function finishInstall(j) {
    ...
    appendJobLog("\n==== " + (ok ? "完成" : "失败") + " ====\n" + msg + "\n");
    if (!ok && j && j.log) appendJobLog(String(j.log).slice(-12000) + "\n");
  ...
  function bindVersionUi() {
    ...
    wire("tm-btn-log", function () {
      ...
      function loadChangelog() {
        attempt += 1;
        api("changelog", null, 18000)
          .then(function (d) {
            showChangelog(d);
          })
          .catch(function (e) {
            if (attempt < 2 && /超时|timeout|502|504|网络|NetworkError|Failed to fetch/i.test(String(e.message || e))) {
              body.innerHTML = "<p>GitHub 响应较慢，正在重试…</p>";
  ...
  function wireFnosTest() {
    ...
    btn.addEventListener("click", function (ev) {
      ...
      if (!values.url || !values.user) { out.textContent = "请先填写飞牛音乐地址和用户名"; return; }
      btn.disabled = true;
      out.textContent = "正在连接…";
      ...
      fetch("/plugins/theme.music/ucwc-music-api.php?action=fnos_test&_ts=" + Date.now(), {
        ...
        out.textContent = j && j.ok ? "连接正常" : ((j && j.error) || "连接失败");
      }).catch(function () { out.textContent = "连接请求失败"; }).then(function () { btn.disabled = false; });
    });
  }
  ...
  window.addEventListener("orientationchange", refreshControlLayout);
  try {
    if (window.jQuery) {
