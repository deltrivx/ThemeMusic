...
(function (global) {
  ...
  function enabled() {
    ...
    var on =
      ...
      en === "true" ||
      en === "on";
    return on && (c.source || "local") === "local";
  }
  ...
  function shouldShowCard() {
    return enabled() && isDashboard() && !state.collapsed && wantCardUi();
  }
  ...
  function onDocumentHidden(reason) {
    try {
      // Always pause when document is hidden: Unraid is full-document nav, so
      // next page resumes via PLAY_KEY intent. Fixes mobile swipe-away keep-play.
  ...
  function rebindCardEls() {
    ...
    try {
      ...
      els.infoPop = root.querySelector(".ucwc-music-info-pop");
  ...
  function closeTrackInfoPop() {
    if (!root) return;
    try {
      root.classList.remove("ucwc-music-info-open");
      if (els.infoBtn) els.infoBtn.setAttribute("aria-expanded", "false");
    } catch (e0) {}
  }
  ...
  function toggleTrackInfoPop(ev) {
    ...
    if (!root) return;
    var open = !root.classList.contains("ucwc-music-info-open");
    if (open) {
      renderTrackInfoPop();
      root.classList.add("ucwc-music-info-open");
      if (els.infoBtn) els.infoBtn.setAttribute("aria-expanded", "true");
    } else {
      closeTrackInfoPop();
    }
  }
  ...
  function updateSidePanel() {
    ...
    if (els.sideLyricRefetch) {
      els.sideLyricRefetch.hidden = isList;
      els.sideLyricRefetch.style.display = isList ? "none" : "";
    }
  ...
  function updateMeta() {
    ...
    if (!t) {
      ...
      if (els.sub) els.sub.textContent = state.loaded ? "" : "本地音乐";
      clearLyricsView("—");
      clearCoverView();
      state.cover.id = "";
      if (root && root.classList.contains("ucwc-music-info-open")) renderTrackInfoPop();
      return;
    }
  ...
  function updateModeBtns() {
    ...
    if (rp) {
      ...
      /* Keep one SVG; only inject if missing — avoid wiping node mid-click */
      if (!rp.querySelector("svg")) rp.innerHTML = svgIcon("repeat");
      els.repeat = rp;
    }
  }
  ...
  function tickLyricsRaf() {
    ...
    syncLyrics(false);
    try {
      lyricsRaf = requestAnimationFrame(tickLyricsRaf);
    } catch (e1) {
      lyricsRaf = 0;
    }
  }
  ...
  function runCardAction(btn, ev) {
    ...
    if (btn.classList.contains("ucwc-music-close")) {
        ...
        try {
          ...
          var btn = t.closest(
            "#ucwc-music-card .ucwc-music-btn, " +
              ...
              "#ucwc-music-card .ucwc-music-close"
          );
          if (!btn) return;
   ...
   * plugin card mounts. Our card body lives inside the first dashboard row, so
   * Dynamix's stock mixedView() has no later row to hide. Keep the native
   * controls, align the dismiss icon with the header actions, and mirror the
   * chevron state onto the plugin body.
   */
  function bindNativeDashboardControls(card) {
    ...
    try {
      ...
      var firstCell = host.querySelector("tr:first-child > td:first-child");
      var dismiss = null;
      if (firstCell && firstCell.children) {
        for (var ci = 0; ci < firstCell.children.length; ci++) {
          var child = firstCell.children[ci];
          if (
            child &&
            child.tagName === "I" &&
            child.classList.contains("control") &&
            child.classList.contains("tile") &&
            (child.classList.contains("fa-close") || child.classList.contains("fa-times"))
          ) {
            dismiss = child;
            break;
          }
      ...
      if (dismiss && dismiss.parentNode !== controls) {
        dismiss.classList.add("ucwc-music-native-dismiss");
        controls.insertBefore(dismiss, controls.firstChild);
      }
      ...
      if (!arrow.__ucwcMusicOpenCloseBound) {
        arrow.__ucwcMusicOpenCloseBound = true;
  ...
  function buildUi() {
    ...
    root.innerHTML =
      ...
      '      <span class="ucwc-music-tile-subtitle">本地音乐播放器</span>' +
      "    </span>" +
      "  </span>" +
      ...
      '          <button type="button" class="ucwc-music-side-btn rescan" title="重新扫描本地曲库" aria-label="重新扫描曲库"></button>' +
      ...
      "    </div>" +
      "  </div>" +
      '  <div class="ucwc-music-info-pop" role="dialog" aria-label="当前曲目信息" hidden></div>' +
      "</div>";
    ...
    if (els.infoPop) {
      try {
        els.infoPop.hidden = false;
        els.infoPop.removeAttribute("hidden");
      } catch (ePop) {}
    }
    syncSourceLabel();
    updateSidePanel();
    ...
    bindNativeDashboardControls(root);
    if (els.infoPop) {
      els.infoPop.addEventListener("click", function (ev) {
        try {
          ev.stopPropagation();
        } catch (eIp) {}
      });
    }
    /* Outside click / Esc closes track info popover (once per page) */
    if (!global.__UCWC_MUSIC_INFO_DOC_BOUND__) {
      global.__UCWC_MUSIC_INFO_DOC_BOUND__ = true;
      document.addEventListener(
        "click",
        function (ev) {
          var card = liveCardRoot();
          if (!card || !card.classList.contains("ucwc-music-info-open")) return;
          try {
            var t = ev && ev.target;
            if (t && t.closest) {
              if (t.closest(".ucwc-music-info-btn")) return;
              if (t.closest(".ucwc-music-info-pop")) return;
            }
            closeTrackInfoPop();
          } catch (eDoc) {}
        },
      ...
      document.addEventListener("keydown", function (ev) {
        try {
          if (ev && ev.key === "Escape") closeTrackInfoPop();
        } catch (eEsc2) {}
      });
    }
    if (els.sideSearch) {
      ...
      els.sideSearch.addEventListener("keydown", function (ev) {
        try {
          if (ev && ev.key === "Escape") {
  ...
  function boot() {
    ...
    try {
      ...
      window.addEventListener("beforeunload", function () {
        ...
        try {
          if (audio && !audio.paused) onDocumentHidden("beforeunload");
