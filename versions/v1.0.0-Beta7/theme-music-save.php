...
if (isset($_POST["SERVICE"])) {
    ...
    if ($section === "service" || (count($_POST) <= 4 && !isset($_POST["MUSIC_ENABLE"]) && !isset($_POST["MUSIC_LOCAL_DIR"]))) {
        if (!$svc_ok) {
...
tm_log($log_path, "save ok=" . ($ok ? "1" : "0") . " enable=" . $fx["MUSIC_ENABLE"] . " dir=" . $fx["MUSIC_LOCAL_DIR"] . ($svc_out ? " svc=" . $svc_out : ""));
