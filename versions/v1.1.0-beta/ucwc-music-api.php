...
function m_navidrome_response($cfg, $endpoint, $params = [], $timeout = 15) {
...
function m_navidrome_songs($cfg, $limit = 2000) {
...
function m_library_cache_read($root, $limit, $ttl = 120) {
...
if ($action === "navidrome_test") {
    [$resp, $err] = m_navidrome_response($cfg, "ping");
    if (!is_array($resp)) {
        mjson(["ok" => false, "error" => $err !== "" ? $err : "Navidrome 连接失败"], 400);
    }
    [$songs, $songErr] = m_navidrome_songs($cfg, 1);
    if (!is_array($songs)) {
...
if ($action === "list") {
    ...
    if ($source === "navidrome") {
        ...
        $cached = $forceScan ? null : m_library_cache_read($cacheKey, $limitIn, 120);
        if (is_array($cached)) {
    ...
    $cached = $forceScan ? null : m_library_cache_read($root, $limitIn, 120);
    if (is_array($cached)) {
        $tracks = $cached["tracks"];
        $err = "";
        $scanMeta = $cached["scan"];
