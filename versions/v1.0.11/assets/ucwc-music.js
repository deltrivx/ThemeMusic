...
(function (global) {
  ...
  function updateSidePanel() {
    ...
    if (els.sideSearchWrap) {
      els.sideSearchWrap.hidden = !isList;
      els.sideSearchWrap.style.display = isList ? "" : "none";
    }
    if (els.sideRescan) {
  ...
  function buildUi() {
    ...
    root.innerHTML =
      ...
      '          <button type="button" class="ucwc-music-side-btn rescan" title="重新扫描本地曲库" aria-label="重新扫描"></button>' +
    ...
    if (els.sideRescan) els.sideRescan.innerHTML = svgIcon("refresh");
    if (els.sideLyricRefetch) els.sideLyricRefetch.innerHTML = svgIcon("refresh");
