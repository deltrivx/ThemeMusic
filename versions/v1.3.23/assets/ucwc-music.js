...
(function (global) {
  ...
  var LS_KEY_BASE = "ucwc_music_v1";
  var PLAY_KEY_BASE = "ucwc_music_play_v1";
  ...
  var normalStatusTimer = 0;
  var cardActionBusy = false;
  var normalStatusIndex = 0;
  var transientStatusTimer = 0;
  ...
  function sourceScope() {
    var c = rawCfg() || {};
    return String(c.source || "local") + ":" + String(c.library_scope || "default");
  }

  function scopedPlayKey() {
    return scopedStorageKey(PLAY_KEY_BASE) + "_" + sourceScope();
  }

  function scopedLibraryCacheKey() {
    return scopedStorageKey(LIBRARY_CACHE_KEY) + "_" + sourceScope();
  }

  function scopedLsKey() {
    return scopedStorageKey(LS_KEY_BASE);
  }
  ...
  function writePlaySession(obj) {
    try {
      ...
      try {
        sessionStorage.setItem(scopedPlayKey(), payload);
      } catch (e0) {}
      try {
        localStorage.setItem(scopedPlayKey(), payload);
      } catch (e1) {}
    } catch (e) {}
  }
        ...
        if (audio && !audio.paused && !audioHasError(audio)) {
          ...
          tryPlayUnlocked(audio, function (playError) {
            if (isAutoplayPolicyError(playError)) {
              ...
              bindGestureUnlock();
              updatePlayBtn();
              if (isDashboard() && isCoarsePointer()) setStatus("点一下任意处可续播", "transient");
  ...
  function loadPlaySession() {
    ...
    var b = null;
    try {
      a = parsePlaySession(sessionStorage.getItem(scopedPlayKey()));
    } catch (e0) {}
    try {
      b = parsePlaySession(localStorage.getItem(scopedPlayKey()));
    } catch (e1) {}
  ...
  function clearPlaySession() {
    try {
      sessionStorage.removeItem(scopedPlayKey());
    } catch (e0) {}
    try {
      localStorage.removeItem(scopedPlayKey());
    } catch (e1) {}
  }
  ...
  function recoverStalledPlayback(reason) {
    ...
    stallZeroSince = 0;
    try {
      setStatus(isCoarsePointer() ? "恢复播放中…" : "音轨恢复中…", "transient");
    } catch (eS) {}
    pendingSeekTo = 0;
    resumeLockId = "";
    var idx = state.index;
      ...
      try {
        ...
        setTimeout(function () {
          ...
          if (audio && audio.paused && (resumeIntent || sessionWantsResume(loadPlaySession()))) {
            ...
            if (isCoarsePointer()) {
              if (isDashboard()) setStatus("点一下屏幕任意处开始播放", "transient");
  ...
  function startNormalStatusCycle() {
    if (normalStatusTimer) clearInterval(normalStatusTimer);
    normalStatusTimer = 0;
    normalStatusIndex = 0;
    if (!state.loaded || state.statusKind !== "normal") return;
    var messages = normalStatusMessages();
    if (messages.length < 2) return;
    normalStatusTimer = setInterval(function () {
      if (state.statusKind !== "normal") return;
      var current = normalStatusMessages();
      if (!current.length) return;
      normalStatusIndex = (normalStatusIndex + 1) % current.length;
      state.statusKind = "normal";
      state.error = current[normalStatusIndex];
      if (els.status) els.status.textContent = state.error;
      syncSourceLabel();
    }, 4200);
  }
  ...
  function loadLs() {
    try {
      var raw = localStorage.getItem(scopedLsKey());
      if (!raw) return;
      var o = JSON.parse(raw);
  ...
  function sourceReadiness(source) {
    ...
    var c = rawCfg() || {};
    if (source === "local") {
      // An empty local_dir selects ThemeMusic's automatic storage detection;
      // it is a valid configured source and must remain in the cycle.
      return String(c.local_dir || "").trim() !== "" || c.local_ready !== false;
    }
    if (source === "navidrome") {
      return String(c.navidrome_url || "").trim() !== "" &&
        String(c.navidrome_user || "").trim() !== "" &&
        !!c.navidrome_password_set;
    }
    if (source === "fnos") {
      return String(c.fnos_url || "").trim() !== "" &&
        String(c.fnos_user || "").trim() !== "" &&
        !!c.fnos_password_set;
    }
    return false;
  }
  ...
  function cycleMusicSource(ev) {
    ...
    for (var step = 1; step <= order.length; step++) {
      var candidate = order[(index >= 0 ? index + step : step - 1) % order.length];
      if (candidate !== currentSource && sourceReadiness(candidate)) { next = candidate; break; }
    }
    if (!next) {
      setStatus("没有已配置的其他音源");
      return true;
    }
    var token = "";
    try {
    ...
    fetch("/plugins/theme.music/theme-music-save.php", {
      ...
      return fetchList(true).then(function () {
  ...
  function setStatus(msg, kind) {
    ...
    syncSourceLabel();
    if (kind === "normal") startNormalStatusCycle();
    else if (normalStatusTimer) {
      clearInterval(normalStatusTimer);
      normalStatusTimer = 0;
    }
    if (transientStatusTimer) clearTimeout(transientStatusTimer);
    if (kind === "transient" && state.loaded) {
      transientStatusTimer = setTimeout(function () {
        transientStatusTimer = 0;
        if (state.statusKind === "transient") setStatus(libraryStatusText(), "normal");
      }, 3600);
    }
  }
  ...
  function loadLyricsForCurrent(opts) {
    ...
    if (
      ...
      (state.lyrics.lines.length || (state.lyrics.empty && Date.now() - (Number(state.lyrics.emptyAt) || 0) < 15000))
    ) {
      // Keep a short negative cache, then allow a later retry.
    ...
    state.lyrics.empty = true;
    state.lyrics.emptyAt = 0;
    state.lyrics.retryCount = force ? (Number(state.lyrics.retryCount) || 0) + 1 : 0;
    state.lyrics.unsynced = false;
    state.lyrics.lines = [];
    state.lyrics.active = -1;
    ...
    fetch(url, {
      ...
      .then(function (r) {
        return r.text().then(function (text) {
          ...
          if (!r.ok) {
            throw new Error(r.status === 401 || r.status === 403 ? "歌词认证失败（HTTP " + r.status + "）" : "歌词服务 HTTP " + r.status);
          }
          if (!trimmed) throw new Error("歌词服务返回空响应");
          if (/<(?:!doctype\s+html|html|body)\b/i.test(trimmed) || /登录|login|sign in/i.test(trimmed.slice(0, 1200))) {
            throw new Error("歌词服务返回登录页面");
          }
          if (ct && ct.indexOf("json") < 0 && !/^[\[{]/.test(trimmed)) throw new Error("歌词服务返回非 JSON");
          try { return JSON.parse(trimmed); } catch (eJson) { throw new Error("歌词服务返回非 JSON"); }
        });
      })
      .then(function (j) {
        ...
        state.lyrics.emptyAt = norm.length ? 0 : Date.now();
        state.lyrics.id = t.id;
      ...
      .catch(function (err) {
        if (seq !== state.lyrics.seq) return;
        state.lyrics.loading = false;
        clearLyricsView(err && err.message ? err.message : "歌词请求失败");
      });
  }
   ...
   * Pane geometry depends only on the available card width. Track and lyric text
   * are clipped or marquee-scrolled inside the right pane and never resize it.
   */
  function fitDashColumnLayout() {
    try {
      ...
      var inner = 0;
      try {
        inner = main.getBoundingClientRect().width || main.clientWidth || 0;
      } catch (eMeasure) {}
      if (inner < 40) {
        var colW = measureDashColumnWidth();
        inner = colW > 40 ? Math.max(0, colW) : 0;
      }
      var leftW = LEFT_W_IDEAL;
      var rightW = RIGHT_W_MIN;
      var stack = false;
      ...
      if (inner > 0) {
        ...
        if (inner < dualMin) {
          ...
          stack = true;
          leftW = Math.max(0, Math.floor(inner));
          rightW = leftW;
        } else {
          leftW = Math.min(
            LEFT_W_IDEAL,
            Math.max(LEFT_W_MIN, Math.floor(inner - PANE_GAP - RIGHT_W_MIN))
          );
  ...
  function runCardAction(btn, ev) {
    ...
    if (btn.classList.contains("ucwc-music-header-refresh")) {
      if (cardActionBusy) return true;
      cardActionBusy = true;
      setStatus("正在刷新音乐卡片…");
      Promise.resolve(fetchList(true)).finally(function () {
        cardActionBusy = false;
      });
      return true;
    }
  ...
  function buildUi() {
    ...
    root.innerHTML =
      ...
      '      <button type="button" class="ucwc-music-header-btn ucwc-music-header-refresh" title="刷新音乐卡片" aria-label="刷新音乐卡片"><i class="fa fa-fw fa-refresh control" aria-hidden="true"></i></button>' +
  ...
  function saveClientLibraryCache(j) {
    try {
      ...
      if (j.tracks.length > 5000) {
        sessionStorage.removeItem(scopedLibraryCacheKey());
        return;
      }
      sessionStorage.setItem(
        scopedLibraryCacheKey(),
        JSON.stringify({
          v: 3,
  ...
  function restoreClientLibraryCache() {
    try {
      var raw = sessionStorage.getItem(scopedLibraryCacheKey());
      if (!raw) return false;
      ...
      try { sessionStorage.removeItem(scopedLibraryCacheKey()); } catch (e1) {}
      return false;
    }
  }
    ...
    return fetch(apiBase + "?action=list&_ts=" + Date.now() + refresh, {
      ...
      .then(function (pack) {
        ...
        if (!j.ok) {
          ...
          state.listTip = "";
          setStatus(j.error || "无法加载曲库", j.source_configured === false ? "persistent" : "persistent");
          updateMeta();
          renderList();
          return;
        }
