...
function m_library_scan_active(array $state) {
    ...
    $pid = (int)($state["pid"] ?? 0);
    if ($pid > 1 && function_exists("posix_kill")) return @posix_kill($pid, 0);
...
function m_track_normalize(array $item, $source, $fallbackId = "") {
    ...
    if ($source === "local" && $artist === "") {
        $filename = pathinfo($path, PATHINFO_FILENAME);
        if (preg_match("/^(.+?)\s+-\s+(.+)$/u", $filename, $pair)) {
            $title = m_track_text($pair[1]);
            $artist = m_track_text($pair[2]);
        }
    }
    ...
    if ($source === "local") {
        ...
        if ($artist === "" && preg_match("/^(.+?)\s+-\s+(.+)$/u", $title, $pair)) {
            $title = m_track_text($pair[1]);
            $artist = m_track_text($pair[2]);
        }
    }
...
function m_remote_library_fetch(array $cfg, $strategy, $progress = null) {
    ...
    if ($strategy === "fnos") {
        ...
        for ($page = 1; count($tracks) < $maxTracks; $page++) {
            ...
            if (count($tracks) === $before || ($reportedTotal > 0 && count($tracks) >= $reportedTotal)) break;
            // FNOS may cap each response at 50 items even when pageSize=200.
            // When the API reports a larger total, continue paging until that total is reached.
            if ($reportedTotal <= 0 && count($items) < $pageSize) break;
        }
...
function m_local_lyrics_text($path, $root, $fnosPath = false) {
    ...
    $candidates = [$dir . "/" . $base . ".lrc", $dir . "/" . $base . ".txt", $dir . "/lyrics.lrc", $dir . "/lyrics.txt", $dir . "/lyric.lrc", $dir . "/lyric.txt"];
    $rootBase = pathinfo($path, PATHINFO_FILENAME);
    $candidates[] = rtrim($root, "/") . "/" . $rootBase . ".lrc";
    $candidates[] = rtrim($root, "/") . "/" . $rootBase . ".txt";
    foreach ([$dir . "/lyrics", $dir . "/lyric", $dir . "/Lyrics", $dir . "/Lyric"] as $lyricsDir) {
        if (!is_dir($lyricsDir)) continue;
        foreach ([$base . ".lrc", $base . ".txt", "lyrics.lrc", "lyrics.txt", "lyric.lrc", "lyric.txt"] as $name) $candidates[] = $lyricsDir . "/" . $name;
    }
    foreach ($candidates as $candidate) if (is_file($candidate) && is_readable($candidate)) {
        $body = (string)@file_get_contents($candidate);
        if ($body !== "") return $body;
    }
    $normalizeStem = function ($value) {
        $value = preg_replace('/^[0-9]+[.\\-_ ]+/u', '', pathinfo((string)$value, PATHINFO_FILENAME));
        $value = preg_replace('/[\\p{P}\\p{S}\\s]+/u', '', $value);
        return function_exists("mb_strtolower") ? mb_strtolower($value, "UTF-8") : strtolower($value);
    };
    $wanted = $normalizeStem($base);
    foreach (array_unique([$dir, $dir . "/lyrics", $dir . "/lyric", dirname($dir)]) as $scanDir) {
        if (!is_dir($scanDir)) continue;
        $dh = @opendir($scanDir);
        if (!$dh) continue;
        while (($entry = readdir($dh)) !== false) {
            if (!preg_match('/\\.(?:lrc|txt)$/i', $entry) || $normalizeStem($entry) !== $wanted) continue;
            $candidate = $scanDir . "/" . $entry;
            if (is_file($candidate) && is_readable($candidate)) {
                $body = (string)@file_get_contents($candidate);
                if ($body !== "") { closedir($dh); return $body; }
            }
        }
        closedir($dh);
    }
...
function m_online_http_json($url, $timeout = 12) {
    $ch = curl_init($url);
    curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => true, CURLOPT_FOLLOWLOCATION => true, CURLOPT_CONNECTTIMEOUT => 4, CURLOPT_TIMEOUT => $timeout, CURLOPT_HTTPHEADER => ["Accept: application/json", "User-Agent: ThemeMusic/1.3"]]);
    $body = curl_exec($ch);
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
...
function m_online_http_binary($url, &$contentType = "") {
    $ch = curl_init($url);
    curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => true, CURLOPT_FOLLOWLOCATION => true, CURLOPT_CONNECTTIMEOUT => 4, CURLOPT_TIMEOUT => 15, CURLOPT_HTTPHEADER => ["Accept: image/*", "User-Agent: ThemeMusic/1.3"]]);
    $body = curl_exec($ch);
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    $contentType = (string)curl_getinfo($ch, CURLINFO_CONTENT_TYPE);
    curl_close($ch);
    if (!is_string($body) || $code < 200 || $code >= 300 || strlen($body) < 256 || strpos($contentType, "image/") !== 0) return "";
    return $body;
}

function m_atomic_media_write($path, $body) {
    if ($path === "" || !is_string($body) || $body === "") return false;
    $tmp = $path . ".tmp." . getmypid() . "." . bin2hex(random_bytes(4));
...
function m_online_track_query($path, $title = "", $artist = "") {
    $title = m_track_text($title);
    $artist = m_track_text($artist);
    $name = pathinfo((string)$path, PATHINFO_FILENAME);
    if ($title === "") $title = m_track_text($name);
    if ($artist === "" && preg_match('/^(.+?)\\s+[\\-–—－]\\s+(.+)$/u', $name, $pair)) {
        $left = m_track_text($pair[1]);
        $right = m_track_text($pair[2]);
        if (preg_match('/(?:feat\\.?|ft\\.?|with)\\s+/iu', $right) || preg_match('/[\\x{4e00}-\\x{9fff}]/u', $right)) {
            $title = $left;
            $artist = $right;
        }
    }
...
function m_lyrics_query_text($value, $stripVersion = false) {
    $value = trim((string)$value);
    $value = preg_replace('/\\.(?:flac|mp3|m4a|aac|ogg|wav|opus|wma)$/iu', '', $value);
    $value = preg_replace('/^[0-9]+[.\\-_ ]+/u', '', $value);
    $value = str_replace(["！", "（", "）", "，", "－", "　"], ["!", "(", ")", ",", "-", " "], $value);
    if ($stripVersion) $value = preg_replace('/\\s*[\\[(（].*?(?:live|remix|version|edit|acoustic|instrumental|伴奏|现场|现场版).*?[\\])）]\\s*/iu', ' ', $value);
    $value = preg_replace('/\\s+(?:feat\\.?|ft\\.?|featuring)\\s+.+$/iu', '', $value);
    $value = preg_replace('/\\s+/u', ' ', $value);
    return trim($value, " \\t-–—");
}

function m_lyrics_query_candidates($path, $title, $artist) {
    [$pathTitle, $pathArtist] = m_online_track_query($path, $title, $artist);
    $title = m_lyrics_query_text($title !== "" ? $title : $pathTitle);
    $artist = m_lyrics_query_text($artist !== "" ? $artist : $pathArtist);
    $items = [];
    $add = function ($t, $a = "") use (&$items) {
        $t = m_lyrics_query_text($t);
        $a = m_lyrics_query_text($a);
        if ($t === "") return;
        $key = strtolower($t . "\n" . $a);
        foreach ($items as $item) if ($item[2] === $key) return;
        $items[] = [$t, $a, $key];
    };
    $add($title, $artist);
    $add($title);
    $add(m_lyrics_query_text($title, true), $artist);
    if ($pathTitle !== $title || $pathArtist !== $artist) $add($pathTitle, $pathArtist);
    return array_slice($items, 0, 6);
}

function m_lyrics_result_score(array $row, $title, $artist) {
    $rt = m_lyrics_query_text($row["trackName"] ?? $row["name"] ?? $row["title"] ?? "", true);
    $ra = m_lyrics_query_text($row["artistName"] ?? $row["artist"] ?? "", true);
    $qt = m_lyrics_query_text($title, true);
    $qa = m_lyrics_query_text($artist, true);
    $score = 0;
    if ($rt !== "" && $rt === $qt) $score += 8;
    elseif ($rt !== "" && (stripos($rt, $qt) !== false || stripos($qt, $rt) !== false)) $score += 3;
    if ($qa !== "" && $ra !== "" && $ra === $qa) $score += 6;
    elseif ($qa !== "" && $ra !== "" && (stripos($ra, $qa) !== false || stripos($qa, $ra) !== false)) $score += 2;
    if (trim((string)($row["syncedLyrics"] ?? "")) !== "") $score += 2;
    return $score;
}

function m_online_lyrics($path, $title, $artist, $root) {
    $queries = m_lyrics_query_candidates($path, $title, $artist);
    if (!count($queries)) return "";
    $best = null;
    $bestScore = -1;
    foreach ($queries as $query) {
        $params = ["track_name" => $query[0]];
        if ($query[1] !== "") $params["artist_name"] = $query[1];
        $data = m_online_http_json("https://lrclib.net/api/search?" . http_build_query($params));
        if (!is_array($data)) continue;
        foreach ($data as $row) {
            if (!is_array($row)) continue;
            $text = trim((string)($row["syncedLyrics"] ?? ""));
            if ($text === "") $text = trim((string)($row["plainLyrics"] ?? ""));
            if ($text === "") continue;
            $score = m_lyrics_result_score($row, $query[0], $query[1]);
    ...
    if ($best !== null) {
        $safe = m_safe_path_under($root, $path);
        if ($safe === "") return "";
        $target = pathinfo($safe, PATHINFO_DIRNAME) . "/" . pathinfo($safe, PATHINFO_FILENAME) . ".lrc";
        m_atomic_media_write($target, $best . "\n");
        return $best;
    }
    return "";
}

function m_online_cover($path, $title, $artist, $root) {
    [$title, $artist] = m_online_track_query($path, $title, $artist);
    if ($title === "") return "";
    $urls = [];
    $term = trim($title . ($artist !== "" ? " " . $artist : ""));
    $data = m_online_http_json("https://itunes.apple.com/search?" . http_build_query(["term" => $term, "media" => "music", "entity" => "song", "limit" => 5]));
    if (is_array($data) && is_array($data["results"] ?? null)) foreach ($data["results"] as $row) {
        if (!is_array($row) || empty($row["artworkUrl100"])) continue;
        $urls[] = preg_replace("#/100x100bb#", "/1200x1200bb", (string)$row["artworkUrl100"]);
    }
    $query = "recording:" . $title . ($artist !== "" ? " AND artist:" . $artist : "");
    $mb = m_online_http_json("https://musicbrainz.org/ws/2/recording/?" . http_build_query(["query" => $query, "fmt" => "json", "limit" => 5]));
    if (is_array($mb) && is_array($mb["recordings"] ?? null)) foreach ($mb["recordings"] as $recording) {
        if (!is_array($recording)) continue;
        foreach (($recording["release-list"] ?? []) as $release) {
            if (!is_array($release) || empty($release["id"])) continue;
            $urls[] = "https://coverartarchive.org/release/" . rawurlencode((string)$release["id"]) . "/front-1200";
        }
        foreach (($recording["release-group-list"] ?? []) as $group) {
            if (!is_array($group) || empty($group["id"])) continue;
            $urls[] = "https://coverartarchive.org/release-group/" . rawurlencode((string)$group["id"]) . "/front-1200";
        }
    }
    $urls = array_values(array_unique(array_filter($urls)));
    foreach ($urls as $url) {
        $type = ""; $body = m_online_http_binary($url, $type);
        if ($body === "") continue;
        $safe = m_safe_path_under($root, $path);
        if ($safe === "") return "";
        $target = pathinfo($safe, PATHINFO_DIRNAME) . "/" . pathinfo($safe, PATHINFO_FILENAME) . ".jpg";
        if (m_atomic_media_write($target, $body)) return $body;
    }
    return "";
}
...
if ($action === "lyrics") {
    ...
    if (($auto["strategy"] ?? "") === "fnos") {
        ...
        if ($lyrics === "") {
            ...
            if ($lyrics === "") {
                $localPath = m_fnos_local_path($remotePath, $root);
                if ($localPath !== "") {
                    $online = m_online_lyrics($localPath, "", "", $root);
                    if ($online !== "") { $lyrics = $online; $source = "online"; }
                }
            }
        $parsed = m_parse_lyrics_text($lyrics);
    ...
    $source = $lyrics !== "" ? "local" : "";
    if ($lyrics === "" && $rel !== "") {
        $online = m_online_lyrics($rel, "", "", $root);
        if ($online !== "") { $lyrics = $online; $source = "online"; }
    }
    $parsed = m_parse_lyrics_text($lyrics);
    mjson([
...
if ($action === "cover") {
    ...
    if ($rel !== "") {
        $online = m_online_cover($rel, "", "", $root);
        if ($online !== "") {
            header("Content-Type: image/jpeg");
            header("Content-Length: " . strlen($online));
            header("Cache-Control: public, max-age=3600");
            echo $online;
            exit;
        }
