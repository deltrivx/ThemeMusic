...
(function () {
  ...
  function alignCtrlWidth() {
    if (!form) return;
    var names = ["MUSIC_SOURCE", "MUSIC_ENABLE", "MUSIC_DASH_ONLY", "MUSIC_REPEAT", "SERVICE"];
    var probes = [];
    var i, el, n, w;
  ...
  function mountVersionInPageTitle() {
    try {
      ...
      if (existing && existing.querySelector("#tm-btn-check")) return;
  ...
  try {
    if (window.jQuery) {
      window.jQuery(function () {
        ...
        setTimeout(mountVersionInPageTitle, 50);
      });
    }
  } catch (e) {}
})();
