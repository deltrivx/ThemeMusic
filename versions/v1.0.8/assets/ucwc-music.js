...
(function (global) {
  ...
  /**
   * Measure longest track label and set --ucwc-music-right-w dynamically.
   * Hard cap RIGHT_W_MAX (original column width) so sibling dashboard tiles never grow.
   */
  function fitRightPane() {
    try {
      ...
      // Prefer live list-item font (browser/CJK metrics differ a lot from body inherit)
      var fontSize = "12px";
      var lineHeight = "1.35";
      ...
      try {
        var ref =
          ...
          root.querySelector(".ucwc-music-item-txt") ||
          root;
      ...
      // item pad 8+8, gap, 词 badge ~26, scrollbar ~10, CJK fudge
      var need = Math.ceil(textW + 16 + 6 + 26 + 12 + 8);
      var w = Math.max(RIGHT_W_MIN, Math.min(RIGHT_W_MAX, need));
      // Cap by real dashboard column — skip content-sized ancestors (avoid self-lock at current width)
      try {
        var host = document.getElementById("ucwc-music-dash-host");
        var col = host ? host.parentElement : null;
        var cardW = root.getBoundingClientRect().width || 0;
        var guard = 0;
        while (col && guard++ < 6) {
          var colW = col.getBoundingClientRect ? col.getBoundingClientRect().width || 0 : 0;
          var style = window.getComputedStyle(col);
          var display = style ? style.display : "";
          // Prefer table cells / explicit column flex parents wide enough to hold dual-pane
          var looksCol =
            colW >= Math.max(cardW + 24, 420) ||
            display === "table-cell" ||
            (col.tagName && /^T(D|H|BODY)$/.test(col.tagName));
          if (looksCol && colW > 80) {
            // body pad 14*2 + left 286 + gap 14 + slack — never above RIGHT_W_MAX
            var maxByCol = Math.floor(colW - 28 - 286 - 14 - 8);
            if (maxByCol >= RIGHT_W_MIN) w = Math.min(w, Math.min(RIGHT_W_MAX, maxByCol));
            break;
          }
          col = col.parentElement;
        }
      } catch (eCol) {}
      main.style.setProperty("--ucwc-music-right-w", w + "px");
      if (root) root.style.setProperty("--ucwc-music-right-w", w + "px");
    } catch (e0) {}
  }
  ...
  function findDashHost() {
    ...
    // 0) If wrap already sits in a live parent, keep that column — never yank to box1
    ...
    // 2) Unraid multi-column boxes — pick the column with most tiles (not always box1/top-left)
    var colSelectors = [
      "#db-box1",
      "#db_box1",
      "#db-box2",
    ...
    for (i = 0; i < colSelectors.length; i++) {
      try {
        ...
        var score = el.children ? el.children.length : 0;
        // Prefer columns that already have widgets; slight preference for later columns
        // so first install does not always land top-left of box1 alone.
        if (i >= 2) score += 0.5;
        if (score > bestScore) {
          bestScore = score;
          best = el;
  ...
  function isStrongDashPos(o) {
    ...
    if (o.prevId || o.nextId) return true;
  ...
  function saveDashPos() {
    try {
      ...
      if (!dashPosServerLoaded && idx === 0 && !prev && kids.length > 1) {
        return;
      }
      var payload = {
        index: idx,
  ...
  function insertDashWrap(host, wrap) {
    ...
    if (wrap.parentNode === host) {
      ...
      if (saved0.prevId) {
        try {
          var p0 = document.getElementById(saved0.prevId);
          if (p0 && p0.parentNode === host && wrap.previousElementSibling !== p0) atOk = false;
        } catch (eA) {}
      }
      if (atOk) return;
    ...
    if (!placed && saved && typeof saved.index === "number" && saved.index >= 0 && kids.length) {
      ...
      if (at === 0 && !saved.prevId && !saved.nextId && kids.length > 2 && saved.total && saved.total > 2) {
        host.appendChild(wrap);
      } else if (at >= kids.length) {
        host.appendChild(wrap);
      } else {
        host.insertBefore(wrap, kids[at]);
      }
      placed = true;
    }
  ...
  function placeInDashboard(card) {
    ...
    if (wrap && wrap.parentNode && wrap.parentNode.isConnected) {
      ...
      try {
        ...
        if (
          ...
          (savedLive.index > 0 || savedLive.prevId || savedLive.nextId)
        ) {
