...
function m_wake_source_path($root, $candidate) {
    ...
    if ($candidate === "" || strpos($candidate, $root . "/") !== 0) {
        return ["ok" => false, "mode" => "invalid"];
    }

    $marker = "/tmp/theme-music-wake-" . sha1($candidate);
    ...
    if (preg_match('#^/mnt/disk(\d+)(?:/|$)#', $candidate, $m)) {
        ...
        $section = "disk" . $m[1];
        if (is_array($ini) && isset($ini[$section]["device"])) {
            $device = trim((string)$ini[$section]["device"]);
        }
    } elseif (strpos($candidate, "/mnt/disks/") === 0 && is_executable("/bin/findmnt")) {
        $cmd = "/bin/findmnt -T " . escapeshellarg($candidate) . " -n -o SOURCE 2>/dev/null";
        $src = trim((string)@shell_exec($cmd));
        if (preg_match('#^/dev/([a-zA-Z0-9._-]+)#', $src, $m)) {
            $device = preg_replace('/p?\d+$/', '', $m[1]);
        }
    }

    if ($device !== "" && preg_match('/^[a-zA-Z0-9._-]+$/', $device) && is_executable("/usr/local/sbin/emcmd")) {
        @exec("/usr/local/sbin/emcmd cmdSpinup=" . escapeshellarg($device) . " >/dev/null 2>&1");
    }

    $fp = @fopen($candidate, "rb");
    if (!$fp) {
        return ["ok" => false, "mode" => $device !== "" ? "spinup-failed" : "probe-failed"];
    }
    $probe = @fread($fp, 1);
    @fclose($fp);
    if ($probe === false) {
        return ["ok" => false, "mode" => "probe-failed"];
    }
    @touch($marker);
    return ["ok" => true, "mode" => $device !== "" ? "unraid-spinup" : "filesystem-probe"];
}
...
function m_navidrome_songs($cfg, $limit = 2000, $passwordOverride = null) {
...
/**
 * Bounded local library scan.
 *
 * Never follow symlinks and never perform per-track directory rescans for
 * covers/LRC files.  Each directory is read once and the walk has hard track,
 * directory, entry and wall-clock budgets so a very large or cyclic share
 * cannot monopolise emhttp or exhaust a low-memory Unraid host.
 *
 * @return array{0:?array,1:string,2:array} [tracks|null, error, meta]
 */
function m_scan($root, $max = 1000) {
    $out = [];
    $root = rtrim(str_replace("\\", "/", (string)$root), "/");
    $max = max(100, min(1200, (int)$max));
    $maxDepth = 6;
    $maxDirs = max(200, min(1200, (int)ceil($max / 2) + 100));
    $maxEntries = max(5000, min(24000, $max * 20));
    ...
    try {
        while (!$queue->isEmpty()) {
            ...
            foreach ($dirIt as $file) {
                ...
                if ($entriesScanned >= $maxEntries) {
                    $truncated = true;
                    $stopReason = "entry_budget";
                    break 2;
                }
                if (($entriesScanned & 63) === 0 && microtime(true) >= $deadline) {
                    $truncated = true;
                    $stopReason = "time_budget";
                    break 2;
                }
        ...
        return [null, $e->getMessage(), [
            "truncated" => false, "limit" => $max, "max_depth" => $maxDepth,
...
function m_library_cache_path($root, $limit) {
    return "/tmp/theme-music-library-" . sha1((string)$root . "\0" . (string)$limit) . ".json";
}

function m_library_cache_read($root, $limit, $ttl = 21600) {
    $path = m_library_cache_path($root, $limit);
    if (!is_file($path) || (time() - (int)@filemtime($path)) > max(5, (int)$ttl)) return null;
    $data = json_decode((string)@file_get_contents($path), true);
    if (!is_array($data) || !isset($data["tracks"]) || !is_array($data["tracks"])) return null;
    return $data;
}

function m_library_cache_write($root, $limit, $tracks, $scanMeta) {
    $path = m_library_cache_path($root, $limit);
    $data = [
...
if ($action === "list") {
    ...
    if ($source === "navidrome") {
        $cacheKey = "navidrome|" . ($cfg["MUSIC_NAVIDROME_URL"] ?? "") . "|" . ($cfg["MUSIC_NAVIDROME_USER"] ?? "");
        $cached = $forceScan ? null : m_library_cache_read($cacheKey, $limitIn, 21600);
        if (is_array($cached)) {
            $tracks = $cached["tracks"];
            $scanMeta = $cached["scan"];
            $fromCache = true;
        } else {
            [$tracks, $err, $truncated] = m_navidrome_songs($cfg, $limitIn);
            if (!is_array($tracks)) {
                mjson(["ok" => false, "error" => "Navidrome 曲库读取失败：" . $err, "tracks" => []], 400);
            }
            $scanMeta = [
                "truncated" => (bool)$truncated,
                "limit" => max(1, min(5000, (int)$limitIn)),
                "max_depth" => 0,
                "count" => count($tracks),
            ];
            m_library_cache_write($cacheKey, $limitIn, $tracks, $scanMeta);
            $fromCache = false;
        }
        ...
        mjson([
            ...
            "max_depth" => 0,
            "cached" => $fromCache,
            "tip" => $truncated ? "Navidrome 曲库已达到加载上限 {$limit} 首，可在设置中缩小曲库范围。" : "",
        ]);
    }
    $root = m_realpath_dir($cfg["MUSIC_LOCAL_DIR"] ?? "");
    if ($root === "") {
    ...
    // Local disks/shares are deliberately bounded more tightly than Navidrome.
    // A normal page load reuses the six-hour server cache; only the explicit
    // rescan button bypasses it.
    $limitIn = isset($_GET["limit"]) ? intval($_GET["limit"]) : 1000;
    $limitIn = max(100, min(1200, $limitIn));
    $cached = $forceScan ? null : m_library_cache_read($root, $limitIn, 21600);
    if (is_array($cached)) {
        $tracks = $cached["tracks"];
        $err = "";
        $scanMeta = $cached["scan"];
        ...
        $fromCache = false;
        if (is_array($tracks)) m_library_cache_write($root, $limitIn, $tracks, $scanMeta);
    }
    if ($tracks === null) {
    ...
    mjson([
        ...
        "cached" => $fromCache,
        "tip" => $truncated
            ? ("安全扫描已停止：已加载 " . count($tracks) . " 首（上限 {$limit}），目录已达到资源预算。请缩小本地音乐目录后手动重扫。")
            : "",
    ]);
}
