...
(function (global) {
  ...
  function syncLyrics(force) {
    if (!audio) return;
    var now = Date.now();
    // Tighter cadence for better A/V lock while scrubbing / mid-track
    if (!force && now - lyricsSyncLast < 80) return;
    lyricsSyncLast = now;
    var hasLines = state.lyrics.lines && state.lyrics.lines.length;
    // LRC offset is typically "delay ms" (positive = lyrics early → subtract).
    // Also apply a small lead (~80ms) so highlight anticipates sung line slightly.
    var off = typeof state.lyrics.offsetMs === "number" ? state.lyrics.offsetMs : 0;
    var tMs = Math.floor((audio.currentTime || 0) * 1000) - off + 80;
    if (tMs < 0) tMs = 0;
    ...
    if (els.lyricsScroll && hasLines) {
      ...
      var lines = state.lyrics.lines || [];
      // Demote previous active (stop marquee)
