...
(function (global) {
  ...
  function fitRightPane() {
    try {
      ...
      // Measure with same font metrics as list items (12px + badge room)
      var probe = document.createElement("span");
      probe.style.cssText =
        "position:absolute;left:-99999px;top:0;visibility:hidden;white-space:nowrap;" +
        "font-size:12px;line-height:1.35;font-family:inherit;font-weight:400;padding:0;";
      ...
      // item pad 8+8, gap, 词 badge ~22, scrollbar ~8
      var need = Math.ceil(textW + 16 + 6 + 22 + 10);
      var w = Math.max(RIGHT_W_MIN, Math.min(RIGHT_W_MAX, need));
      // Also never exceed remaining room inside host column (if measurable)
      try {
        var host = document.getElementById("ucwc-music-dash-host");
        var col = host && host.parentElement ? host.parentElement : null;
        if (col && col.getBoundingClientRect) {
          var colW = col.getBoundingClientRect().width || 0;
          if (colW > 80) {
            // body pad 14*2 + left 286 + gap 14 + small slack
