...
(function (global) {
  ...
  function libraryStatusText() {
    ...
    base += " · " + listSortLabel(state.listSort);
  ...
  function buildUi() {
    ...
    if (els.sideRescan) {
      els.sideRescan.addEventListener(
        "click",
        btnOnly(function () {
          setStatus("正在重新扫描…");
          fetchList();
    ...
    if (els.sideLyricRefetch) {
      els.sideLyricRefetch.addEventListener(
        "click",
        btnOnly(function () {
    ...
    if (els.infoBtn) {
      els.infoBtn.addEventListener("click", btnOnly(toggleTrackInfoPop));
    }
    if (els.infoPop) {
    ...
    /* Outside click / Esc closes track info popover */
    document.addEventListener(
      "click",
      function (ev) {
        if (!root || !root.classList.contains("ucwc-music-info-open")) return;
        try {
          var t = ev && ev.target;
          if (els.infoBtn && els.infoBtn.contains(t)) return;
          if (els.infoPop && els.infoPop.contains(t)) return;
          closeTrackInfoPop();
        } catch (eDoc) {}
      },
      true
