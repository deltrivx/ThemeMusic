...
(function (global) {
   ...
   * Card itself is width:100% like other Unraid tiles; only internal L/R split is dynamic.
   * Right hard-capped at RIGHT_W_MAX (280); never expands the column / siblings.
   */
  function fitDashColumnLayout() {
    try {
      ...
      var rightIdeal = Math.max(RIGHT_W_MIN, Math.min(RIGHT_W_MAX, contentNeed));
      var leftW = LEFT_W_IDEAL;
      var rightW = rightIdeal;
      var stack = false;

      if (inner > 0) {
        // Budget for both panes + gap inside the tile body
        var dualMin = LEFT_W_MIN + PANE_GAP + RIGHT_W_MIN;
        if (inner < dualMin + 8) {
          // Column too narrow for side-by-side (typical tight 3-col) — stack, full width each
          stack = true;
          leftW = Math.max(LEFT_W_MIN, Math.floor(inner));
          rightW = Math.max(RIGHT_W_MIN, Math.min(RIGHT_W_MAX, Math.floor(inner)));
        } else {
          // Prefer ideal left; give remaining to right (capped 280, floored by content/min)
          var maxRight = Math.min(RIGHT_W_MAX, Math.max(RIGHT_W_MIN, inner - LEFT_W_MIN - PANE_GAP));
          rightW = Math.max(RIGHT_W_MIN, Math.min(maxRight, rightIdeal));
          leftW = Math.max(LEFT_W_MIN, Math.min(LEFT_W_IDEAL, inner - PANE_GAP - rightW));
          // If leftover after left ideal, grow right up to cap (fill column like other tiles' content)
          var leftover = inner - leftW - PANE_GAP - rightW;
          if (leftover > 4 && rightW < RIGHT_W_MAX) {
            rightW = Math.min(RIGHT_W_MAX, rightW + leftover);
          }
          // Rebalance if still overflowing
          if (leftW + PANE_GAP + rightW > inner) {
            rightW = Math.max(RIGHT_W_MIN, inner - leftW - PANE_GAP);
            if (rightW > RIGHT_W_MAX) {
              rightW = RIGHT_W_MAX;
              leftW = Math.max(LEFT_W_MIN, inner - PANE_GAP - rightW);
            }
          }
        }
      }
