...
> 当前版本：**v1.3.14** · 插件 ID：`theme.music` · 最低 Unraid：**6.12.0**

## 为什么选择 Theme Music

| 能力 | 说明 |
|---|---|
| 三类音源 | 读取 Unraid 本地目录，或通过 Subsonic API 接入 Navidrome/OpenSubsonic，也支持 FnOS 音乐 HTTP API |
...
sh /tmp/theme-music-install.sh install v1.3.12 ota
```
...
- 本地和远端大曲库由低优先级单实例后台任务建立完整索引，Navidrome 按 500 首、FnOS 按 200 首分页读取；页面始终复用最后一次完整结果，重复刷新不会叠加扫描。
- 曲目列表按 300 首分段渲染，300 首只是界面渲染批次，不是曲库或播放上限；完整索引不再因单页或前端截断而丢失。歌词可按曲目提前/延后 0.5 秒并手动归零。
